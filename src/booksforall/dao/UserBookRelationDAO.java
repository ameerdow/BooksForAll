...
public class UserBookRelationDAO {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			if (statement.executeUpdate() == 0) {
				throw new SQLException(
						"Could not add new user ( " + user.getUsername() + " book " + book.getID() + " relation like");
			} else {
				Log.l(classFunc, "AddUserBookLike",
						"New user ( " + user.getUsername() + " book " + book.getID() + " relation like added");
			}
			connection.commit();
		} catch (Exception e) {
	...
	public void deleteUserBookLike(User user, Book book) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			if (statement.executeUpdate() == 0) {
				throw new SQLException(
						"Could not delete user ( " + user.getUsername() + " book " + book.getID() + " relation like");
			} else {
				Log.l(classFunc, "deleteUserBookLike",
						"user ( " + user.getUsername() + " book " + book.getID() + " relation like deleted");
			}
			connection.commit();
		} catch (Exception e) {
	...
	public UserBookLikeRelation getUserBookLikeRelation(User user, Book book) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			if (rs.next()) {
				return new UserBookLikeRelation(rs.getString("USERNAME"), rs.getInt("BOOK_ID"),
						rs.getString("NICKNAME"));
			} else
				throw new RuntimeException(
						"There was not relations for user " + user.getUsername() + " and book " + book.getID());
	...
	public List<UserBookLikeRelation> getUserBooksLikeByUser(User user) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			ResultSet rs = statement.executeQuery();
			while (rs.next()) {
				userBookLikeRelationList.add(new UserBookLikeRelation(rs.getString("USERNAME"), rs.getInt("BOOK_ID"),
						rs.getString("NICKNAME")));
			}
		} catch (Exception e) {
	...
	public UserBookReviewRelation addUserBookReview(User user, Book book, String review) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			PreparedStatement statement = connection.prepareStatement(ADD_USER_BOOK_REVIEW,
			...
			if (statement.executeUpdate() == 0) {
				...
				if (generatedKeys.next())
					return new UserBookReviewRelation(generatedKeys.getInt(1), user.getUsername(), book.getID(), review,
							"N", date.toString());
				else
	 ...
	 * Approve user review
	 *
	 * @param bookReviewRelation
	 *            user book relation object
	 */
	...
	public List<UserBookReviewRelation> getPendingReview() {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			while (rs.next()) {
				reviewRelations.add(new UserBookReviewRelation(rs.getInt(1), rs.getString(2), rs.getInt(3),
	...
	public UserBookReviewRelation getReviewByID(int reviewID) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			ResultSet rs = statement.executeQuery();
			if (rs.next()) {
				return new UserBookReviewRelation(rs.getInt("ID"), rs.getString("USERNAME"), rs.getInt("BOOK_ID"),
						rs.getString("REVIEW"), rs.getString("APPROVED"), rs.getString("SYS_CREATION_DATE"));
			}
		} catch (Exception e) {
			Log.e(classFunc, "approveUserBookReview", e.getMessage(), e);
		}
	...
	public List<UserBookReviewRelation> getAllBookReviews(Book book) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			while (rs.next()) {
				userBookReviewRelationList.add(new UserBookReviewRelation(rs.getInt(1), rs.getString(2), rs.getInt(3),
						rs.getString(4), rs.getString(5), rs.getString(6)));
			}
		} catch (Exception e) {
	...
	public void buyBook(User user, Book book) {
		...
		if (userBookPurchaseRelation == null) {
			try (Connection connection = new DBConnection().getConnection()) {
				...
				if (statement.executeUpdate() == 0) {
					throw new RuntimeException(
							"Error purchasing book " + book.getID() + " for username " + user.getUsername());
				}
				connection.commit();
			} catch (Exception e) {
	...
	public UserBookPurchaseRelation getUserBookPurchaseRelation(User user, Book book) {
		...
		if (user != null && user.getUsername() != null && !user.getUsername().isEmpty() && book != null
	...
	public List<UserBookPurchaseRelation> getAllPurchases() {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			while (rs.next()) {
				userBookPurchaseRelationList.add(new UserBookPurchaseRelation(rs.getString("USERNAME"),
						rs.getInt("BOOK_ID"), rs.getDouble("PRICE"), rs.getDate("SYS_CREATION_DATE")));
			}
		} catch (Exception e) {
	...
	public UserBookPositionRelation getUserBookPosition(String username, int bookId) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			ResultSet rs = statement.executeQuery();
			if (rs.next()) {
				return new UserBookPositionRelation(username, bookId, rs.getFloat("POSITION"),
	...
	public void addUserBookPosition(String username, int bookId, Float position) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			if (statement.executeUpdate() == 0) {
				Log.l(classFunc, "addUserBookPosition", "error adding position, username " + username + " , bookId "
						+ bookId + " position " + position);
	...
	public void saveUserBookPosition(String username, int bookId, Float position) {
		...
		try (Connection connection = new DBConnection().getConnection()) {
			...
			if (statement.executeUpdate() == 0) {
				Log.l(classFunc, "saveUserBookPosition", "error saving position, username " + username + " , bookId "
						+ bookId + " position " + position);
