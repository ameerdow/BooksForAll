 ...
 */
@WebServlet(name = "BookServlet", description = "book servlet manage all book operations", urlPatterns = { "/books",
		"/book/*", "/search/book/" })
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (uri.endsWith(GET_ALL_BOOKS)) {
				printWriter.println(gson.toJson(bookService.getAllBooks()));
				return;
				...
				for (UserBookLikeRelation userBookLikeRelation : likeRelations) {
				...
				for (UserBookReviewRelation reviewRelation : reviewRelations) {

					reviewsList.add(new ReviewResponse(reviewRelation.getUsername(),
							userService.getUserByUsername(reviewRelation.getUsername()).getNickname(),
							reviewRelation.getReview()));
				}
				String username = Helper.checkSession(request);
				List<UserBookPurchaseRelation> purchaseRelations = bookService.getAllPurchasesForBook(bookId);
				for (UserBookPurchaseRelation purchaseRelation : purchaseRelations) {
					if (purchaseRelation.getUsername().equals(username)) {
						book.setPurchased(true);
					}
				}
				book.setUserDetailsList(likers);
				book.setReviews(reviewsList);
				UserBookPositionRelation r = userService.getReadPosition(username, bookId);
				book.setPosition(r != null && r.getPositionPercent() != null ? r.getPositionPercent() : 0);
				...
				printWriter.println(gson.toJson(bookService.getBookByCategory(category)));
				return;
				...
				printWriter.println(gson.toJson(bookService.getAllBookLikes(bookId)));
				return;
				...
				int bookId = Integer.parseInt(params[1]);
				printWriter.println(gson.toJson(bookService.getAllReviewsForBook(bookId)));
				return;
				...
				int bookId = Integer.parseInt(params[2]);
				printWriter.println(gson.toJson(bookService.getAllPurchasesForBook(bookId)));
				return;
				...
				printWriter.println(gson.toJson(userService.getPurchasedBooksByUsername(username)));
				return;
				...
				String username = params[3];
				printWriter.println(gson.toJson(userService.getNotPurchasedBooksByUsername(username)));
				return;
				...
				String username = Helper.checkSession(request);
				int bookId = Integer.parseInt(params[2]);
				printWriter.println(gson.toJson(userService.getReadPosition(username, bookId)));
				return;
			}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (uri.contains(DELETE_BOOK_BY_ID)) {
				ClientRequest.DeleteBookRequest deleteBookRequest = gson.fromJson(postData,
						ClientRequest.DeleteBookRequest.class);
				...
				ClientRequest.BuyBookRequest buyBookRequest = gson.fromJson(postData,
						ClientRequest.BuyBookRequest.class);
				...
				ClientRequest.LikeBookRequest likeBookRequest = gson.fromJson(postData,
						ClientRequest.LikeBookRequest.class);
				...
				ClientRequest.ReviewBookRequest reviewBookRequest = gson.fromJson(postData,
						ClientRequest.ReviewBookRequest.class);
				if (Helper.checkSession(request).isEmpty() || reviewBookRequest.getBookId() == 0
						|| reviewBookRequest.getReview().isEmpty())
					throw new RuntimeException("username is empty or bookId is 0 or review is empty");
				userService.reviewBook(Helper.checkSession(request), reviewBookRequest.getBookId(),
						reviewBookRequest.getReview());
			} else if (uri.contains(SAVE_READ_BOOK_POSITION)) {
				ClientRequest.SetReadPositionRequest readPositionRequest = gson.fromJson(postData,
						ClientRequest.SetReadPositionRequest.class);
				if (Helper.checkSession(request).isEmpty() || readPositionRequest.getBookId() == 0
						|| readPositionRequest.getPosition().isNaN())
					throw new RuntimeException("username is empty or bookId is 0 or position is null");
				userService.saveReadPosition(Helper.checkSession(request), readPositionRequest.getBookId(),
						readPositionRequest.getPosition());
			} else {
				throw new RuntimeException("no function found");
			}
		} catch (RuntimeException e) {
